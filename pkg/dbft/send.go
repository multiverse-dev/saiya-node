...
func (d *DBFT) sendPrepareRequest() {
	...
	d.Logger.Info("sending PrepareRequest", zap.Uint32("height", d.BlockIndex), zap.Uint("view", uint(d.ViewNumber)), zap.Int("tx_count", len(msg.GetPrepareRequest().TransactionHashes())))
	d.changeTimer(delay)
	d.checkPrepare()
}
